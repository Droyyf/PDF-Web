...
class PDFComposerApp {
    ...
    async generateAllThumbnails() {
        if (!this.currentPDF || this.isCancelled) {
            console.log('Skipping thumbnail generation - cancelled or no PDF');
            return;
        }
        ...
        try {
            ...
            this.isProcessing = false;
            if (this.isCancelled) {
                console.log('Thumbnail generation completed with cancellation');
            }
        }
    }
    ...
    async createNewCustomOverlayExportCanvas(scaleFactor = 2) {
        ...
        // Get preview canvas dimensions for consistent scaling
        const previewCanvas = document.getElementById('previewCanvas');
        let baseWidth, baseHeight;
        
        if (!previewCanvas) {
            console.warn('Preview canvas not found - using fallback dimensions');
            baseWidth = 800;
            baseHeight = 1131;
        } else {
        ...
        // Render the composition using the same logic as preview
    ...
    async createCompositionCanvas(scaleFactor = 2) {
        ...
        // For side-by-side mode, use standard export dimensions
        if (this.overlayMode === 'side-by-side') {
            // Use standard dimensions that work well with 60/40 split
            baseWidth = 1200;  // Standard width for side-by-side export
            baseHeight = 800;  // Standard height for side-by-side export
            
            console.log('Side-by-side mode: using standard canvas dimensions', baseWidth, 'x', baseHeight);
        } else {
    ...
    async renderSideBySideToCanvas(context, canvasWidth, canvasHeight) {
        ...
        // Calculate 60/40 split dimensions
        const citationSectionWidth = canvasWidth * 0.6;
        const coverSectionWidth = canvasWidth * 0.4;
        
        // Calculate scales for each section to fit within their allocated space
        const citationScale = Math.min(
            citationSectionWidth / citationViewport.width,
        ...
        // Render citation page (60% section)
        const citationScaledViewport = citationPage.getViewport({ scale: citationScale });
        ...
        console.log('Rendering citation page', firstCitationPageIndex, 'at position (', citationX, ',', citationY, ') with scale', citationScale);
        ...
        console.log('Rendering cover page', this.selectedCover, 'at position (', coverX, ',', coverY, ') with scale', coverScale);
        ...
        const separatorX = citationSectionWidth;
        context.strokeStyle = '#ddd';
        context.lineWidth = 1;
        context.beginPath();
    ...
    cancelAllOperations() {
        console.log('🛑 Executing cancelAllOperations...');
        ...
        // Clear PDF and buffer references
        this.currentPDF = null;
    ...
    async renderSideBySidePreview() {
        ...
        try {
            ...
            // Calculate available viewport space for responsive scaling
            const viewportWidth = window.innerWidth;
            const viewportHeight = window.innerHeight;
            
            // Reserve space for header and controls (estimated)
            ...
            // Calculate responsive scaling to fit viewport perfectly
            ...
            // Find optimal scale to fit within viewport
            let optimalScale = 1;
            ...
            // Calculate individual scales for each page
            let citationScales = [];
            for (const viewport of citationViewports) {
                citationScales.push(optimalScale);
            }
            const coverWidthScale = optimalScale;
            
            // Set canvas dimensions to fit viewport responsively
            ...
            // Render citation pages with responsive scaling
            let currentX = 0;
